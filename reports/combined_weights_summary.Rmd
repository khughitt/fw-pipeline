...
```{r results = 'asis'}
# plot distribution of gene set sizes prior to filtering
gene_set_sizes <- NULL

for (collection in names(gene_set_collections)) {
  set_sizes <- unlist(lapply(gene_set_collections[[collection]], length))
  gene_set_sizes <- rbind(gene_set_sizes, data.frame(collection, n = set_sizes))
}
colnames(gene_set_sizes) <- c('collection', 'n')

gene_set_sizes %>%
  group_by(collection) %>%
  summarize(min_size = min(n), median_size = median(n), max_size = max(n))
```

```{r}
# plot gene set size distributions, split across several plots
num_plts_per_image <- 9

for (i in seq_len(ceiling(length(gene_set_collections) / num_plts_per_image))) {
  start_ind <- (i - 1) * num_plts_per_image + 1
  end_ind   <- min(length(gene_set_collections), i * num_plts_per_image)

  plt_dat <- gene_set_sizes %>%
    filter(collection %in% names(gene_set_collections)[start_ind:end_ind])

  plt <- ggplot(plt_dat, aes(x = n, fill = collection)) +
    geom_density() +
    facet_wrap(~collection, scales = 'free') +
    geom_vline(xintercept = config$max_size, color = 'red', linetype = 'dashed') +
    theme_bw() +
